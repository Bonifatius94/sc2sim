...
class QLAgent:
    ...
    gamma: float=0.99
    model: Dict[Tuple[int, int], np.ndarray] = field(default_factory=dict)
    action_space: int=8
    step: int=0
    ...
    @staticmethod
    def _encode_state(state: SC2State) -> Tuple[int, int]:
        return (state.beacon_pos[0] - state.marine_pos[0],
                state.beacon_pos[1] - state.marine_pos[1])
